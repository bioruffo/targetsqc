...
class Genedata(object):
    # genes = Genedata(config.genes, config.bed, config.annotation, over, genesbed)
...
class Config(object):
    def __init__(self, configfile = "selector.cfg", section = "FILES"):
        parser = ConfigParser()
        parser.read(configfile)
        try:
            self.sectiondata = parser[section]
        except KeyError as e:
            print("\n***\nERROR: configuration file invalid, or not found.\n***\n")
            raise e
    
    def __getattr__(self, attr):
        return self.sectiondata.get(attr, None)



def report(config, run_params, genes, data,
...
def main():
    ...
    suffix = "test"
    platform = "Proton"
    filter_quality = indel
    min_mapping_qv = 4
    min_failed_size = 2 # ignore 1-bp dels
    over = 10 # add this many nucleotides on each side of exons and CDS
    configfile = "/home/roberto/Dropbox/Scripts/Selector/exome.cfg"
    bam = ""
    wannovar = ""
    select = True
    
    # Output file names
    reportfile = "selector_report_{}.tsv".format(suffix)
    selectfile = "selector_output_{}.csv".format(suffix)
    genesbed = "selector_genes_{}.bed".format(suffix)
    failedbed = "selector_failed_{}.bed".format(suffix)
    ...
    data = NGSData(config.wannovar, config.bam, genes)

    report(config, run_params, genes, data, reportfile) # TODO add "over"
    if select:
